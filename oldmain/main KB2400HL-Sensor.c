...
static	void	Slice0_Exec( void )
{
	...
	//	日均A
	Output_EN = Read_BitN( ucRegCoilsBuf, 20 );	// 允许输出?
	...
	//	日均B
	Output_EN = Read_BitN( ucRegCoilsBuf, 25 );	// 允许输出?
...
static	void	Slice1_Exec( void )
{
	...
	//	调试
	usRegInputBuf[32] = rint ( 16.0f * get_NTC1_Temp());	//	NTC1 温度
	usRegInputBuf[37] = rint ( 16.0f * get_NTC2_Temp());	//	NTC2 温度
}
